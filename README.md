...
> **Current Stage:** Motor Cascade Dual-Loop Position Control (外环 PD + 内环 PI) + Pendulum Swing-Up & Balancing
>
...
项目硬件平台采用 PID 入门套件，目前已完成**电机串级双环位置控制系统**（外环 PD + 内环 PI）以及**倒立摆摆起与平衡控制系统**（能量法摆起 + 外环位置 PID + 内环角度 PID 级联）。系统通过层次化状态机实现菜单导航，支持以下工作模式：

- **定速模式（SPEED）**：固定 PID 参数，通过 K1/K2 按键调节目标速度，K3 急停
- **定位模式（POSITION）**：串级双环控制（外环位置 PD → 内环速度 PI），电位器 RP4 旋钮实时控制目标位置，速度上限可调
- **倒立摆模式（PENDULUM）**：能量法自动摆起 → 双环 PID 平衡保持，子状态机管理摆起/平衡/倾倒保护全流程
...
├── PendulumTask    — 倒立摆控制（摆起 + 平衡），200Hz
├── UITask          — OLED 显示，10Hz
...
| **PendulumTask** | `osPriorityNormal` | 5ms (200Hz) | 512×4 B | 倒立摆控制：4 状态子状态机（IDLE → SWING_UP → BALANCING → FALLEN）→ 能量法摆起 + 外环位置 PD + 内环角度 PID 级联平衡；维护倒立摆全局变量（`pendulum_*` 系列）；通过 volatile 标志位接收 FsmTask 命令 |
...
- **摆起阶段**：能量法——检测摆杆角度和角速度，计算当前能量与目标能量的差值，控制电机往复运动注入能量，直到摆杆接近竖直后自动切入 BALANCING
...
倒立摆模式通过 4 状态子状态机管理完整的摆起→平衡→保护流程。

**子状态机：**

```text
IDLE ──K1──→ SWING_UP ──(自动)──→ BALANCING ──(倾倒)──→ FALLEN
  ▲              │                    │                    │
  │              │ 超时               │ K1                 │ 1.5s
  │              ▼                    ▼                    │
  └──── K1 ── FALLEN ◄───────────────┘ ◄──────────────────┘
...
| **SWING_UP** | 能量法自动摆起，通过往复运动积累能量直到摆杆接近竖直（带超时保护） |
...
```text
PENDULUM Spd:XX
aKp:0.80 aKi:0.12 aKd:0.08
pKp:0.05 pKi:0.00 pKd:0.02
...
| 按键 | 功能 |
| ---- | ---- |
| K1 | 启动 / 停止摆控制（IDLE ↔ SWING_UP / 运行中急停） |
| K2 | 顺时针旋转一圈（仅在 BALANCING 状态） |
| K3 | 逆时针旋转一圈（仅在 BALANCING 状态） |
| K4 短按 | 返回主菜单 |
...
│   │   │   ├── PendulumTask.c           # 倒立摆控制任务：摆起 + 平衡（200Hz，优先级 Normal）
...
- **PENDULUM Mode** — 4-state sub-FSM (IDLE → SWING_UP → BALANCING → FALLEN). Energy-based swing-up auto-transitions to cascade dual-loop balancing (outer position PD + inner angle PID at 200 Hz). K1 toggles start/stop, K2/K3 rotate one revolution, K4 short returns to main menu, K4 long enters DEBUG. Uses `PID_PositionalPosition()` (outer) + `PID_PositionalAngle()` (inner).
...
| PendulumTask | `osPriorityNormal` | 5ms (200Hz) | 4-state sub-FSM: energy swing-up → cascade dual-loop balancing (outer position PD + inner angle PID); maintain `pendulum_*` globals; receive commands via volatile flags |
...
**Pendulum mode** uses cascade dual-loop control at 200 Hz: the outer loop is position PD (Ki=0) — position error produces an angle offset added to the vertical target (~2048). The inner loop is angle PID — angle error drives PWM to maintain balance. Swing-up uses an energy-based method: the controller computes current vs. target energy and drives the motor reciprocally to inject energy until the pendulum nears vertical, then auto-transitions to balancing.
...
- **PENDULUM Mode:** 4-state sub-FSM (IDLE→SWING_UP→BALANCING→FALLEN); K1 toggles start/stop, K2/K3 rotate ±1 revolution, K4 short → main menu, K4 long → DEBUG; OLED shows dual-ring PID params (angle + position) in afont8x6
